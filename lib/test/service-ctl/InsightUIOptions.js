...
const NodeJsServiceOptions = require('@quantisnetevo/dp-services-ctl/lib/services/node/NodeJsServiceOptions');
...
class InsightUIOptions extends NodeJsServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    const port = this.getRandomPort(20002, 29796);
    ...
    const defaultServiceOptions = {
      config: {
        ...
        services: [
          'quantisnetd',
          '@quantisnetevo/insight-api',
          '@quantisnetevo/insight-ui',
          'web',
        ],
      },
    ...
    const defaultContainerOptions = {
      image: 'quantisnetpay/insight',
      network: {
        name: 'quantisnet_test_network',
        driver: 'bridge',
      },
      ports: [
        `${port}:${port}`,
      ...
      cmd: [
        '/insight/bin/quantisnetcore-node',
        'start',
      ],
      volumes: [...InsightUIOptions.defaultCustomOptions.container.volumes,
        `${configFile}:/insight/quantisnetcore-node.json`,
      ],
    };
  ...
  getConfigPath() {
    ...
    return path.join(tmpPath, 'bitcore-node-quantisnet.json');
  }
